...
class EnhancedSequenceGraphEncoder(nn.Module):
    """Enhanced version with multiple aggregation options"""

    def __init__(self, hidden_size, num_layers=1, dropout=0.1, aggregation='last'):
        super(EnhancedSequenceGraphEncoder, self).__init__()
        self.hidden_size = hidden_size
        self.aggregation = aggregation

        self.encoder = SequenceGraphEncoder(hidden_size, num_layers, dropout)

        # Additional aggregation layers if needed
        if aggregation == 'attention':
            self.attention = nn.MultiheadAttention(hidden_size, num_heads=1)
            self.agg_proj = nn.Linear(hidden_size, hidden_size)

    def forward(self, item_sequence, item_embeddings, seq_lengths=None):
        seq_rep, node_reps = self.encoder(item_sequence, item_embeddings)

        # Get all sequence node representations
        alias_inputs = self.encoder.build_graph_from_sequence(item_sequence, item_embeddings)[3]
        seq_nodes = node_reps[alias_inputs]  # [seq_len, hidden_size]

        # Apply different aggregation strategies
        if self.aggregation == 'last':
            # Use last node as in basic version
            final_rep = seq_nodes[-1]
        elif self.aggregation == 'mean':
            # Mean pooling over sequence
            final_rep = seq_nodes.mean(dim=0)
        elif self.aggregation == 'max':
            # Max pooling over sequence
            final_rep, _ = seq_nodes.max(dim=0)
        elif self.aggregation == 'attention':
            # Self-attention over sequence nodes
            seq_nodes = seq_nodes.unsqueeze(1)  # [seq_len, 1, hidden_size]
            attn_output, _ = self.attention(seq_nodes, seq_nodes, seq_nodes)
            final_rep = self.agg_proj(attn_output[-1, 0, :])  # Use last with projection
        else:
            final_rep = seq_rep

        return final_rep, node_reps
