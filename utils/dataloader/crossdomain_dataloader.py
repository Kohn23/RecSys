...
#
#
# class AbstractCrossDomainDataloader:
#     """
#     This is a new Abstract Dataloader for Cross Domain Recommender
#
#     Note:
#         Do not inherit from torch.utils.data.Dataloader
#     """
...
#         raise NotImplementedError("Method [update_config] must be implemented.")
#
...
#
#
# class CrossDomainDataloader(AbstractCrossDomainDataloader):
#     """
#     This is a fixing based on the new abstract dataloader
#
#     Args:
#         config (Config): The config of dataloader.
...
#             return self._BothStateIterator(self.source_dataloader, self.target_dataloader)
#         else:
#             return self
#
#     def __next__(self):
#         if self.state == CrossDomainDataLoaderState.SOURCE:
#             return next(iter(self.source_dataloader))
#         elif self.state == CrossDomainDataLoaderState.TARGET:
#             return next(iter(self.target_dataloader))
#         elif self.state == CrossDomainDataLoaderState.OVERLAP:
#             return next(iter(self.overlap_dataloader))
#         else:
#             raise StopIteration("BOTH should utilize")
#
#     class _BothStateIterator:
#         def __init__(self, source_dataloader, target_dataloader):
#             self.source_iterator = iter(source_dataloader)
...
#         def __next__(self):
#             try:
#                 source_data = next(self.source_iterator)
#             except StopIteration:
#                 self.source_iterator = iter(self.source_dataloader)
#                 source_data = next(self.source_iterator)
#
#             target_data = next(self.target_iterator)
#             target_data.update(source_data)
#             return target_data
#
#     def __len__(self):
